...
seda_smart_query() {
	usage "${FUNCNAME[0]}" 2 "$#" "TARGET_CONTRACT" "QUERY_MSG"
...
seda_wasm_execute() {
	usage "${FUNCNAME[0]}" 4 "$#" "TARGET_CONTRACT" "EXECUTE_MSG" "FROM" "AMOUNT"
...
# Requires: SEDA_BINARY, jq, SEDA_CHAIN_RPC, SEDA_CHAIN_ID
seda_store_contract() {
	usage "${FUNCNAME[0]}" 2 "$#" "CONTRACT_NAME_PATH" "FROM"
	check_commands $SEDA_BINARY jq
...
# Usage: instantiate_contract CODE_ID INSTANTIATE_MSG FROM ADMIN LABEL
# Requires: SEDA_BINARY, jq, SEDA_CHAIN_RPC, SEDA_CHAIN_ID
seda_instantiate_contract() {
	usage "${FUNCNAME[0]}" 5 "$#" "CODE_ID" "INSTANTIATE_MSG" "FROM" "ADMIN" "LABEL"
	check_commands $SEDA_BINARY jq
	check_env_vars SEDA_CHAIN_RPC SEDA_CHAIN_ID
	local output=$($SEDA_BINARY tx wasm instantiate $1 $2 --from $3 --admin $4 --node $SEDA_CHAIN_RPC --label "$5" ${TXN_GAS_FLAGS} -y --output json --chain-id $SEDA_CHAIN_ID)
	local TXHASH=$(echo "$output" | jq -r '.txhash')
	echo "Transaction Hash: $TXHASH"
...
seda_migrate_call() {
	usage "${FUNCNAME[0]}" 4 "$#" "OLD_CONTRACT_ADDRESS" "NEW_CODE_ID" "MIGRATION_MSG" "FROM"
	check_commands $SEDA_BINARY
	check_env_vars SEDA_CHAIN_RPC SEDA_CHAIN_ID
